...
import { createSquarePayment, isSquareServerConfigured } from '@/lib/square-server';
...
function requiresSquareCheckout(preparedOrder) {
  return preparedOrder.type === 'Shipping' && preparedOrder.payment === 'Card';
}
...
function prepareOrderInput(order) {
  ...
  const paymentRequest =
    order?.paymentRequest && typeof order.paymentRequest === 'object'
      ? {
          ...
          sourceId: String(order.paymentRequest.sourceId || '').trim()
        }
      : null;
  ...
  if (paymentRequest?.provider && paymentRequest.provider !== 'square') {
...
function createDemoOrderRecord({ preparedOrder, ipAddress }) {
  ...
  const pickupDeadline = preparedOrder.type === 'Pickup' ? getPickupDeadlineIso(createdAt) : null;
  const usesSquare = requiresSquareCheckout(preparedOrder);
  ...
  return {
    ...
    order: {
      ...
      paymentProvider: usesSquare ? 'square' : 'manual',
      paymentStatus: usesSquare ? 'DEMO_AUTHORIZED' : getManualPaymentStatus(preparedOrder),
      paymentReferenceId: usesSquare ? orderId : null,
      ...
      customer: {
        ...
        address: preparedOrder.type === 'Pickup' ? 'Pickup arranged in demo mode' : preparedOrder.customer.address.trim(),
...
function getSquareAmountInCents(total) {
...
function buildOrderPayload({
  ...
  const payload = {
    ...
    customer_address: preparedOrder.type === 'Pickup' ? 'Pickup arranged in Bakersfield' : preparedOrder.customer.address.trim(),
    notes: preparedOrder.customer.notes?.trim() || null,
    order_type: preparedOrder.type,
    payment_method: preparedOrder.payment,
    total: preparedOrder.total,
    status: 'Pending',
    pickup_deadline: preparedOrder.type === 'Pickup' ? getPickupDeadlineIso(createdAt) : null,
  ...
  return {
    ...
    square_payment_id: paymentMetadata.squarePaymentId || null,
    square_order_id: paymentMetadata.squareOrderId || null,
  ...
  if (usesSquare) {
    if (!isSquareServerConfigured()) {
      throw createHttpError(
        ...
        'Square checkout is not configured yet. Add the Square application, location, and access token values first.'
      );
    }

    if (preparedOrder.paymentRequest?.provider !== 'square' || !preparedOrder.paymentRequest.sourceId) {
      throw createHttpError(400, 'SQUARE_SOURCE_REQUIRED', 'A Square card token is required before submitting this order.');
    }

    const squareOrderPayload = buildOrderPayload({
      preparedOrder,
      accountRow,
      orderNumber,
      createdAt,
    ...
    const { data: insertedOrderRow, error: insertOrderError } = await client
      .from('store_orders')
      .insert(squareOrderPayload)
      .select('*')
      .single();
    ...
    try {
      const payment = await createSquarePayment({
        sourceId: preparedOrder.paymentRequest.sourceId,
        amount: getSquareAmountInCents(preparedOrder.total),
        idempotencyKey: randomUUID(),
        referenceId: orderNumber,
        note: `Home Grown ${orderNumber} shipping order`,
        buyerEmailAddress: preparedOrder.email,
        buyerPhoneNumber: preparedOrder.customer.phone.trim()
      });

      const paymentStatus = payment?.status || 'UNKNOWN';
      const paymentUpdate = {
        payment_status: paymentStatus,
        payment_reference_id: payment?.referenceId || orderNumber,
      ...
      if (paymentStatus !== 'COMPLETED') {
        await client
          ...
          .eq('id', insertedOrderRow.id);

        throw createHttpError(502, 'SQUARE_PAYMENT_INCOMPLETE', 'Square did not complete the card payment.');
      }

      const { data: updatedOrderRow, error: updateOrderError } = await client
        .from('store_orders')
      ...
      return {
        backendReady: true,
        order: serializeOrderRow(updatedOrderRow),
        account: serializeAccountRow(accountRow)
      };
    } catch (squareError) {
      const failedStatus = squareError.code === 'SQUARE_PAYMENT_INCOMPLETE' ? 'INCOMPLETE' : 'FAILED';
      ...
      if (squareError.code && squareError.status) {
        throw squareError;
      }

      throw createHttpError(502, 'SQUARE_PAYMENT_FAILED', squareError.message || 'Square payment could not be completed.');
    }
  }
